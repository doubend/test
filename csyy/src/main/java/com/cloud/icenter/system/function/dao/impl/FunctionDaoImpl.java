package com.cloud.icenter.system.function.dao.impl;
...
public class FunctionDaoImpl extends BaseDaoImpl<Function> implements
	...
	public void update(Function obj) {
		...
		if (function.getFuncId().equals(obj.getFuncId())) {
	...
	private Function getChild(String parentId, String name) {
		List<Function> functions = getCriteria()
				...
				.add(Restrictions.eq("status", Function.STATUS_DEFAULT)).list();
	...
	public void delete(String id) {
		...
		for (Function f : functions) {
			if (f.getType() == Function.TYPE_LEAF) { // 如果是叶子节点,那么直接删除,否则是目录节点,需要递归删除所有子节点
				super.delete(f.getFuncId());
			} else {
				delete(f.getFuncId());
			}
			executeUpdate("delete RoleFunction where functionId=?", f.getFuncId());
			executeUpdate("delete OrganFunction where functionId=?", f.getFuncId());
		}
		executeUpdate("delete RoleFunction where functionId=?", id);
		executeUpdate("delete OrganFunction where functionId=?", id);
		// add by YHT
		executeUpdate("delete MenuFunction where functionId=?", id);
		super.delete(id);
	}
	...
	@Override
	public List<Function> getChildrenForRole(String roleId, String parentId,
			Integer status) {
	...
	@Override
	public List<Function> getChildrenForOrgan(String organId, String parentId,
			Integer status) {
		...
		DetachedCriteria subquery = DetachedCriteria
				.forClass(OrganFunction.class);
		subquery.add(Restrictions.eq("organId", organId));
		subquery.setProjection(Projections.property("functionId"));
		criteria.add(Subqueries.propertyIn("funcId", subquery));
	...
	public void move(String targetId, String sourceId, String point) {
		...
		if (point.equals("append")) { // 如果是追加操作,那么直接追加到父节点下
			source.setParentId(target.getFuncId());
	...
	public List<Function> getChildrenForMenu(String menuId, String parentId,
		...
		DetachedCriteria subquery = DetachedCriteria
				.forClass(MenuFunction.class);
		subquery.add(Restrictions.eq("menuId", menuId));
		subquery.setProjection(Projections.property("functionId"));
		criteria.add(Subqueries.propertyIn("id", subquery));
