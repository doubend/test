...
public class OrganAuthDaoImpl extends BaseDaoImpl implements AuthDao {
	...
	private boolean checkUrl(List<Organ> organs, String url) {
		...
		if (organs == null || organs.isEmpty() || StringUtils.isBlank(url))
			return false;
	...
	public void refreshCache() {
		...
		subquery.setProjection(Projections.distinct(Projections
				.property("organId")));
		List<Organ> organs = getCriteria(Organ.class).add(
				Subqueries.propertyIn("id", subquery)).list();

		List<Organ> os = new ArrayList<Organ>(1);
		for (Menu m : menus) {
			if (m.getType() == Menu.TYPE_DIRECTORY
					|| m.getStatus() == Menu.STATUS_DISABLE) {
				continue;
			}
			for (Organ o : organs) {
				os.add(o);
	...
	private void addMenuCache(Organ organ, Menu menu) {
		...
		if (!hasCache(organ, menu)) {
			...
			cache.setDataId(organ.getOrgId());
	...
	private boolean hasCache(Organ organ, Menu menu) {
		...
		String key = organ.getOrgId() + "|" + menu.getMenuId();
	...
	private List<Organ> getOrgans(OnlineUser loginUser) {
		if (loginUser == null || loginUser.getParents() == null)
			return null;
	...
	public List<Function> checkFunction(OnlineUser loginUser, String parentId) {
		List<Organ> organs = getOrgans(loginUser);
		if (organs == null || organs.isEmpty())
			return Collections.EMPTY_LIST;

		Query query = null;
		if (parentId != null) {
			query = getNamedQuery("system.checkFunctionForOrgan");
			query.setString("parentId", parentId);
		} else {
			query = getNamedQuery("system.checkTopFunctionForOrgan");
		}
		query.setParameterList("organId", toIdArray(organs));
