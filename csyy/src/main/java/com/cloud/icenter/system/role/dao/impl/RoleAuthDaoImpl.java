...
public class RoleAuthDaoImpl extends BaseDaoImpl implements AuthDao {
	...
	public void init() {
		...
		transactionTemplate.execute(new TransactionCallbackWithoutResult() {
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) {
				String guestRole = SystemConfig
						.getProperty("system.role.guest");
				if (!StringUtils.isBlank(guestRole)) {
					guest = roleDao.getRoleByName(guestRole);
				}
				String developerRole = SystemConfig
						.getProperty("system.role.developer");
	...
	private boolean checkUrl(List<Role> roles, String url) {
		...
		if (roles == null || roles.isEmpty() || StringUtils.isBlank(url))
			return false;
		if (developer != null && roles.contains(developer))
			return true; // 如果有developer角色,那么直接通过,不需要验证
	...
	private void addMenuCache(Role role, Menu menu) {
		...
		if (!hasCache(role, menu)) {
			...
			cache.setDataId(role.getRoleId());
	...
	private boolean hasCache(Role role, Menu menu) {
		...
		String key = role.getRoleId() + "|" + menu.getMenuId();
	...
	public List<Function> checkFunction(OnlineUser loginUser, String parentId) {
		List<Role> roles = getRoles(loginUser);
		if (roles == null || roles.isEmpty())
			return Collections.EMPTY_LIST;

		Query query = null;
		if (parentId != null) {
			query = getNamedQuery("system.checkFunctionForRole");
			query.setString("parentId", parentId);
		} else {
			query = getNamedQuery("system.checkTopFunctionForRole");
		}
		query.setParameterList("roleId", toIdArray(roles));
