...
//初始化区域图的对象
	...
	this.$nodeUrl=null; //节点编辑界面URL
	this.$pluginData=null;//插件数据
	...
	if(property.haveHead){
      ...
      tmp+="<label title='"+(property.initLabelText||"newFlow_1")+"' "
		...
		for(var x=0;x<property.headBtns.length;++x){
			tmp+="<a title='"+ property.headBtnsTit[x] +"' href='javascript:void(0)'" +"' id='"+this.$id+"_btn_"+property.headBtns[x].split(" ")[0]+"' class='GooFlow_head_btn'><i class='ico_"+property.headBtns[x]+"'></i></a>"
		}
		tmp+="</div>";
		this.$head=$(tmp);
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				...
				case "ico_end round": e.data.inthis.switchToolBtn("end round");break;
				//case "ico_start round": e.data.inthis.switchToolBtn("start round");break;//取消原来的运行事件 yht
				case "ico_cursor": e.data.inthis.switchToolBtn("cursor");break;
				case "ico_direct": e.data.inthis.switchToolBtn("direct");break;
			}
			///this.$editable=true;//启用编辑
		});
	}
	var toolWidth=0;
     ...
     +"<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct'><i class='ico_direct'/></a>"
		...
		if(property.toolBtns&&property.toolBtns.length>0){
			...
			for(var i=0;i<property.toolBtns.length;++i){
				tmp+="<a href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_hbtn_"+property.toolBtns[i].split(" ")[0]+"' class='GooFlow_tool_btn'><i class='ico_"+property.toolBtns[i]+"'/></a>";//加入自定义按钮
			}
	...
	if(this.$editable){
	  ...
	  this.onDomdbClick=null;
	  //重写元素的双击事件，修改此双击后，原来的重命名事件将失效 ---add by yht
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
			...
			This.$textArea.val(oldTxt).css({display:"block",width:120,height:14,
				left:t.left+x-This.$workArea[0].parentNode.scrollLeft-$("#"+This.$id).offset().left,
				top:t.top+y-This.$workArea[0].parentNode.scrollTop-$("#"+This.$id).offset().top}).data("id",This.$focus).focus();
	...
	initGroup:function(width,height){
	  ...
	  this.$group.on("mousedown",{inthis:this},function(e){//绑定RESIZE功能以及移动功能
		...
		document.onmousemove=function(e){
			...
			switch(cursor){
				...
				This.$ghost.css({left:X+hack-$("#"+This.$id).offset().left+"px",top:Y+hack-$("#"+This.$id).offset().top+"px"});
			}
			isMove=true;
		}
		document.onmouseup=function(e){
	  ...
	  this.$group.on("dblclick",{inthis:this},function(e){
		...
		This.$textArea.val(oldTxt).css({display:"block",width:100,height:14,
			left:t.left+x-This.$workArea[0].parentNode.scrollLeft-$("#"+This.$id).offset().left,
			top:t.top+y-This.$workArea[0].parentNode.scrollTop-$("#"+This.$id).offset().top}).data("id",p.id).focus();
		This.$workArea.parent().one("mousedown",function(e){
			if(e.button==2)return false;
			if(This.$textArea.css("display")=="block"){
				This.setName(This.$textArea.data("id"),This.$textArea.val(),"area");
	...
	switchToolBtn:function(type){
		...
		this.$head.children("#"+this.$id+"_btn_"+this.$nowType.split(" ")[0]).attr("class","GooFlow_head_btn");//
		...
		this.$head.children("#"+this.$id+"_btn_"+this.$nowType.split(" ")[0]).attr("class","GooFlow_head_btn GooFlow_head_btndown");
		if(this.$nowType=="group"){
			this.blurItem();
	...
	addNode:function(id,json){
	  if(id=="jobdata" || id=="coldata")return;//不处理job数据 add by yht
		...
		// add by yht
		var ico="ico_";
		if(json.type.indexOf(" fa")>0 || json.type.indexOf("font")>0){
			ico="";
		}
		if(json.type.indexOf(" round")<0){
			...
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='"+ico+json.type+"'></i></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		}
		else{
			json.width=34;json.height=34;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><i class='"+ico+json.type+"'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
		}
      ...
      if(mark&&GooFlow.prototype.color.mark){
       ...
       this.$nodeDom[id].addClass("item_mix");
		...
		if(this.$nodeUrl!=null){
			json.nodeUrl=this.$nodeUrl;
		}
		//add by yht 
		if(this.$pluginData!=null){
      json.pluginData=this.$pluginData;
    }
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".ico","mousedown",{inthis:this},function(e){
			...
			document.onmousemove=function(e){
				...
				// 此处减去$("#"+This.$id).offset().left 用以修正偏移量  yht
				// 加上 14px 修正偏移量 gxf
				//This.$ghost.css({left:X+hack-$("#"+This.$id).offset().left+"px",top:Y+hack-$("#"+This.$id).offset().top+"px"});
				This.$ghost.css({left:X+hack-$("#"+This.$id).offset().left+"px",top:Y+hack-$("#"+This.$id).offset().top+14+"px"});
				isMove=true;
			}
			document.onmouseup=function(e){
		...
		this.$workArea.delegate(".GooFlow_item > .span","dblclick",{inthis:this},function(e){
			...
			// add by yht
			if(This.onDomdbClick!=null){
				This.onDomdbClick(id);
				return;
			}
			//-------------end
			...
			This.$textArea.val(oldTxt).css({display:"block",height:$(this).height(),width:100,
				left:t.left+This.$nodeData[id].left-This.$workArea[0].parentNode.scrollLeft-24-$("#"+This.$id).offset().left,
				top:t.top+This.$nodeData[id].top-This.$workArea[0].parentNode.scrollTop+26-$("#"+This.$id).offset().top})
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			...
			This.$textArea.val(oldTxt).css({display:"block",width:$(this).width()+24,height:$(this).height(),
				left:t.left+24+This.$nodeData[id].left-This.$workArea[0].parentNode.scrollLeft-$("#"+This.$id).offset().left,
				top:t.top+2+This.$nodeData[id].top-This.$workArea[0].parentNode.scrollTop-$("#"+This.$id).offset().top})
				.data("id",This.$focus).focus();
			This.$workArea.parent().one("mousedown",function(e){
				if(e.button==2)return false;
				This.setName(This.$textArea.data("id"),This.$textArea.val(),"node");
		...
		this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]","mousedown",{inthis:this},function(e){
			...
			This.$ghost.css({display:"block",
				width:This.$nodeData[id].width-2+"px", height:This.$nodeData[id].height-2+"px",
				top:This.$nodeData[id].top+t.top-This.$workArea[0].parentNode.scrollTop-$("#"+This.$id).offset().top+hack+"px",
				left:This.$nodeData[id].left+t.left-This.$workArea[0].parentNode.scrollLeft-$("#"+This.$id).offset().left+hack+"px",cursor:cursor
			});
			var X,Y;
			X=ev.x-t.left+This.$workArea[0].parentNode.scrollLeft;
			Y=ev.y-t.top+This.$workArea[0].parentNode.scrollTop;
