var animationMgr = function () {
    ...
    var atmMonitor = function () {
        ...
        for (var key in animationList) {
            ...
            if (animationList[key] != "none") {
                ...
                var br = new blockRotater(animationList[key].Id, index == 0 ? maxT : minT);
                animationList[key].blockRotater=br;
            }
            index++;
        }
        //如果没有数据，重试5次
    ...
    return {
            ...
            return {
                ...
                active: function (id, p) {
                    ...
                    if ($("#" + id + ">div").length == 1) {
                       ...
                       animationList[key] ={Id:id,blockRotater:{}};//animationMgr= animationList[key] = id;
                       //alert(key+' '+id)
                }
            };
        },
        stop:function(id){
            for(var key in animationList){
                if(animationList[key] && animationList[key]!="none"){
                    animationList[key].blockRotater.stop();
                }
            }
            //first=undefined;
            //last=undefined;
        },
        reStart:function(paras){
           //setTimeout(atmMonitor, 2000);
            //if(animationList[id] && animationList[id]!="none"){
                //animationList[id].blockRotater.start();
            //}
           for(var key in animationList){
               var id=animationList[key].Id;
