...
var HighChartConfig = {
    plotLines: [
//         { color: 'gray', value: 3000, width: 1 },
//         { color: 'gray', value: 2500, width: 1 },
//         { color: 'gray', value: 2000, width: 1 },
//         { color: 'gray', value: 1500, width: 1 },
//         { color: 'gray', value: 1000, width: 1 },
//         { color: 'gray', value: 500, width: 1 }
        {color: 'gray', value: 1000, width: 1},
        {color: 'gray', value: 800, width: 1},
        {color: 'gray', value: 600, width: 1},
        {color: 'gray', value: 400, width: 1},
        {color: 'gray', value: 200, width: 1}
    ...
    $.fn.extend({
                    ...
                    if (o.chartType == 1) {
                        tempContent.highcharts({
                            ...
                            yAxis: {
                                ...
                                lineColor: o.labelColor,
                                tickInterval: o.interval,
                                plotLines: o.plotLines,
                                min: 0,
                                max: o.max
                            },
                            //tooltip: {
        ...
        rollChart: function (options) {
            ...
            return this.each(function (i) {
                ...
                $.each(opts.data.data, function (j, o) {
                    ...
                    tempUl.append("<li><div class='media list-item'><div class='pull-left'><img src='/yq/img/kbh-laba.png'></div><div class='media-body'><a href=" + o.news_url + " target='_blank' class='whiteu' title='" + o.news_title.trim() + "'><div class='listone'>" + o.news_title.trim() + "</div></a><div class='listtwo'><div class='fleft'><span>" + o.report_date.substr(0, 10) + "</span><span>" + o.report_date.substr(11, 8) + "</span></div><div class='fright'><span>评论(" + commentCount + ")</span><span>转发(" + reportCount + ")</span></div></div></div></div></li>");
                });
        ...
        columnChart: function (options) {
            var defVal = {
                ...
                height: 200,
                labelColor: "#FFFFFF",
                colors: ['#FF5C3D', '#7ACA8F', '#f4bf4d'],
                categories: [],
                series: [],
                legendEnabled: true,
                plotOptionsSeries: {groupPadding: 0.15}
            }
            var opts = $.extend({}, defVal, options);
            ...
            return $this.highcharts({
                ...
                xAxis: {
                    ...
                    labels: {
                        //formatter: function () {
                        //    return this.value.replace(/市/g, '').replace(/省/g, '').substr(0,4);
                        //}
                    }
                },
                yAxis: {
        ...
        // 国元舆情排行
        stackedColumnChart: function (options) {
            var defVal = {
                title: "",
                height: 200,
                labelColor: "#FFFFFF",
        ...
        // 国元健康度条形图
        healthyChart: function (options) {
            var defVal = {
                title: "",
                height: 150,
                labelColor: "#FFFFFF",
                colors: ['#FF5C3D', '#7ACA8F'],
                categories: [],
                series: []
            }
            var opts = $.extend({}, defVal, options);
        ...
        spiderChart: function (options) {
            ...
            return $this.highcharts({
                ...
                tooltip: {
                    formatter: function () {
                        var tmp = this.y == 70 ? '&lt;=70%' : this.y + '%';
                        return '<span style="color:"' + this.series.color + '">' + this.x + '：<b>' + tmp + '</b></span>';
                    }
                },
                plotOptions: {
                    area: {
...
//                    }
        ...
        // 国元舆情走势图
        lineTrendChart: function (options) {
            var defVal = {
                height: 100,
                colors: ['#7ACA8F', '#FF5C3D'],
                title: "",
                marginTop: 0,
                categories: [],
                data: []
        ...
        lineChart2: function (options) {
            ...
            return $this.highcharts({
                ...
                yAxis: {
                    ...
                    gridLineWidth: 1,
                    gridLineColor: "#2E2D35",
                    tickInterval: opts.interval,
                    min: 0,
                    max: opts.max
                },
                tooltip: {
                    formatter: function () {
                        var s = '<b>' + Highcharts.dateFormat('%d', this.x) + '/' + this.y + '</b>';
                        return s;
                    },
                    borderWidth: 0,
                    crosshairs: false
                ...
                series: [
                    {
                        name: '数量',
                        type: opts.lineType == null ? "spline" : opts.lineType,
                        data: opts.data,
                        lineWidth: 1,
                        color: opts.splineColor,
                        marker: {
                            fillColor: "rgba(0, 0, 0, 0.7)",
                            lineColor: opts.splineColor,
                            lineWidth: 1
                        },
                        //type : "area",
                        fillColor: {
        ...
        pieChart2: function (options) {
            ...
            var opts = $.extend({}, defVal, options);
            var tempContent = $("<div class='" + opts.contentCSS + " resizeChart'></div>");
            tempContent.addClass(opts.contentCSS);
            tempContent.height(opts.height);
            opts.data.length = opts.data.length > 6 ? 6 : opts.data.length;
            $.each(opts.data, function (i1, o1) {
                var css = "";
                var newo1 = o1;
                if (o1.length > 4) {
                    css = "font-size:16px;padding-top:20px;"
            ...
            return $this.highcharts({
                ...
                plotOptions: {
                    area: {
                        stacking: 'normal',
                        series: {
                            lineColor: ['#f8f8f8', '#ce754b', '#7bc3de'],
                            fillColor: ['#f8f8f8', '#ce754b', '#7bc3de']
                        },
                        lineWidth: 1,
                        marker: {
                            lineWidth: 1,
        ...
        turnChart2: function (options) {
            ...
            $(this).empty();
            var tempContent = $("<div class='" + opts.contentCSS + " resizeChart'></div>");
            var ulId = $this.attr("id") + "marqueesUL";
            var ul = $("<ul id='" + ulId + "'></ul>");
            $.each(opts.data, function (i1, o1) {
                var li = $("<li><div class='fl-content " + (o1.score == 0 ? 'nt_yellow' : (o1.score > 0 ? 'nt_green' : 'nt_red')) + "'><a href='" + o1.news_url + "' target=\"_blank\" class='white' title='" + o1.news_title.trim() + "'><span>" + o1.news_title.trim() + "</span></a></div><span class=\"data\">" + formatDate(o1.report_date).substr(5, 5) + "</span></li>");
                ul.append(li);
            ...
            //滚动
            var marqueeObja = new marqueeObj()
            marqueeObja.append(ulId, ul.find("li").height() + 5, 1200);
