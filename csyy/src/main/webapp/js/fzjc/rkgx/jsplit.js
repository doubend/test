jQuery.fn.extend({
    jsplit: function (j) {
        return this.each(function () {
            ...
            jQuery(_self).wrapInner("<div class='jsplit-c' style='top:0px;z-index:9999;zoom:1;width:100%;position:relative;height:100%'></div>");
            jQuery(_self).children(".jsplit-c").append("<div class='jsplit-e' unselectable='on' style='background:#fff;height:40px;width:10px;top:50%;-moz-user-select:none;" + antiD + ":-10px;position:absolute;cursor:e-resize;overflow:hidden;z-index:10000;'><div class='jsplit-e-handle'  unselectable='on'  style='height:40px;width:100%;top:50%;margin-top:-20px;left:0;position:absolute;cursor:pointer;-moz-user-select:none;'></div></div>");
            
            var dw = jQuery(_self).width();
            var contW = jQuery(j.ContId).width();
            ...
            function _selfclose() {
                ...
                jsplith.css({ background: j.Btn.cBg.Out, "background-image": j.BgUrl });
                jQuery(_self).animate({ width: "10px" }, 400, function(){
                    $("#tabsLeft").hide();
                    jQuery(_self).addClass("leftBorder");
                    /** 控制右侧容器宽度 Aramey **/
                    var mainWidth = jQuery(_self).parent().width();
                    var leftContWidth = mainWidth - jQuery(_self).outerWidth() -1;
                    //console.log(leftContWidth);
                    jQuery(j.ContId).css('width', leftContWidth);
                    j['Fn'] && j['Fn'].call(_self);
                });
