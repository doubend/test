var reload = function() {
	window.$('#jiuYeTables').datagrid('reload');
...
// 查询无记录时的提示信息
var msg = function() {
	var length = $('#jiuYeTables').datagrid('getRows').length;
	if (length == 0) {
...
var formatterView = function(value,rec,rowIndex){
	return '<a href="javascript:void(0)" onclick="viewCoworker(\''+rec.zwxm+'\',\''+rec.gzdwzzjgdm+'\',\''+rec.sfzhm+'\')"><span style="color: blue;">查看同事关系</span></a></a>'
}
...
//查看同事关系
function viewCoworker(name,zzjgdm,sfz){

	var url = 'peoplebasicquery/toTongShiGuanXi.do?zzjgdm=' + zzjgdm + '&sfz=' + sfz;
	window.parent.addTab('tt','同事关系'+'【'+name+':'+zzjgdm+'】',url);
}