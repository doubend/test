		...
		Namespace('system.user', {
			...
			update: function () {
				...
				getDialog('update-dialog').dialog({
					href: contextPath + '/system/user/update/' + row.userId,
					title: '修改用户',
					width: 500,
					height: 320,
					buttons: [{
			...
			doUpdate: function () {
				...
				$.postJson(contextPath + '/system/user/update/' + row.userId,
			...
			doAdd: function () {
				...
				$.postJson(contextPath + '/system/user/add', data, function (
					result) {
					$('#datagrid').datagrid('loaded');
					if (result.code == 200) {
						top.showInfo('添加成功!');
			...
			remove: function () {
				...
				$.messager.confirm('信息提示', '确定要删除吗？', function (sure) {
					if (!sure)
						return;
					$('#datagrid').datagrid('loading');
					$.post(contextPath + '/system/user/delete/' + row.userId,
						function (result) {
							$('#datagrid').datagrid('loaded');
							if (result.code == 200) {
								top.showInfo('删除成功!');
			...
			updatePassword: function () {
				...
				getDialog('password-dialog').dialog({
					href: contextPath + '/system/user/password/' + row.userId,
					title: '修改密码',
					width: 350,
					height: 250,
					buttons: [{
			...
			doUpdatePassword: function () {
				if (!$('#password-form').form('validate'))
					return;
				...
				$('#datagrid').datagrid('loading');
				var row = $('#datagrid').datagrid('getSelected');
				$.post(contextPath + '/system/user/password/' + row.userId,
					data, function (result) {
						$('#datagrid').datagrid('loaded');
						if (result.code == 200) {
							top.showInfo('修改密码成功!');
